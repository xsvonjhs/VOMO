...
public class VideoPlaybackBehaviour : MonoBehaviour
{
    ...
    [SerializeField]
    [HideInInspector]
    private Texture mKeyframeTexture = null;   //"http://www.pictalive.com/icon/Star.png";
    ...
    void Start()
    {
        ...
        mVideoPlayer.SetFilename(m_path, sponsor, sponsorName, NumberOfFrame );
...

        {
            ...
            float aspect = videoWidth / (float)videoHeight;// / (float)videoWidth;

            // Flip the plane as the video texture is mirrored on the horizontal <---This flips it
            transform.localScale = new Vector3(-0.0f, 0.1f, 0.1f * 0.1f*aspect);
        }
        //*******
        */
    ...
    private IEnumerator LoadVideo()
    {
        ...
        // Load the video
        if (mVideoPlayer.Load(m_path, mMediaType, false, 0, sponsor, sponsorName, NumberOfFrame))
        {
            yield return new WaitForEndOfFrame();
